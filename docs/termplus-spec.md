# Termplus — Modern Cross-Platform SSH Client
...
**Termplus** to nowoczesny, cross-platformowy (Linux, Windows, macOS) klient SSH
i manager połączeń zdalnych inspirowany designem i UX aplikacji **Termius**.

Aplikacja stawia na: czysty, minimalistyczny interfejs z ciemnym motywem jako domyślnym,
płaską nawigację opartą o **Vaults → Hosts → Groups → Tags**, poziome taby połączeń,
wbudowany SFTP jako osobny tab, **Snippets** (zapisane komendy), **Command Palette**
(Ctrl+K), **Keychain** (zarządzanie kluczami SSH) i **Port Forwarding** z GUI.

W odróżnieniu od Termius: Termplus jest **open-source**, **nie wymaga konta ani chmury**
(wszystko lokalnie), i dodatkowo wspiera **RDP, VNC, Telnet i Serial**.

**Motto:** *„Nowoczesny terminal, zero subskrypcji."*

---

## 2. Inspiracja Termius — kluczowe wzorce UX
...
### 2.4 Styl UI
- Dark theme domyślny (ciemne tło #1a1a2e / #16213e, akcenty #0f3460 / #e94560)
- Zaokrąglone rogi, subtelne cienie, smooth animacje przejść
- Fonty: Inter/IBM Plex Sans (UI), JetBrains Mono/Fira Code (terminal)
- Ikony: Lucide lub Phosphor (outline style)
- Sidebar po lewej: collapsible, z sekcjami Vault
...
|---------|-------------|--------------|
| **GUI Framework** | **PySide6** (Qt 6, LGPL) + heavy QSS custom styling | Cross-platform; QSS pozwala na „Termius-like" look |
...
- Auto-save (jak Termius — nie trzeba klikać „Save")
...
SFTP w Termplus to osobna sekcja (jak w Termius) — nie sidebar, a pełnoprawna
strona z tabami sesji SFTP.
...
- **Import/Export:** import z ~/.ssh/config, PuTTY, MobaXterm, Termius JSON, CSV; export encrypted/plaintext JSON
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│  [⚡ Termplus]    [ Vault ]  [ Connections (3) ]  [ SFTP ]  [ PF ]      │
│                                                          [⚙] [👤] [─×] │
├──────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│  === VAULT VIEW ===                                                      │
...
- Tokyo Night / Day
- One Dark / One Light
- Termplus Default (custom, ciepłe kolory)
...
- **Session logs:** opcjonalne, per-host, zapisywane lokalnie w `~/.termplus/logs/`
...
- [ ] Custom QSS dark theme — „Termius-like" look
...
├── termplus.db              # SQLite — cały stan (hosts, keys, snippets, history)
...
```
Jesteś senior Python developerem budującym Termplus — nowoczesny,
cross-platformowy SSH client inspirowany designem Termius.

Stack: Python 3.12+, PySide6 (Qt6) z heavy custom QSS styling,
paramiko, pyte, pyserial, asyncio + qasync.

Design guidelines:
- UI ma wyglądać jak Termius: dark theme, zaokrąglone rogi, smooth transitions,
  nowoczesne ikony (Lucide), fonty Inter (UI) + JetBrains Mono (terminal).
...
7. Auto-save: zmiany w Host Editor zapisują się automatycznie (jak Termius).
...
*Specyfikacja Termplus v1.0 — wygenerowana 2026-03-26*
