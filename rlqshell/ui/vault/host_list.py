...
from rlqshell.ui.widgets.badge import StatusBadge
from rlqshell.ui.widgets.empty_state import EmptyState
from rlqshell.ui.widgets.tag_widget import TagPill

logger = logging.getLogger(__name__)

# Protocol display colors (functional, not brand — used in proto badge & filter chips)
...
_DRAG_MIME = "application/x-rlqshell-host-id"


class _FilterChip(QPushButton):
    """Toggleable filter chip button — flat outline design matching TagPill."""
...
class HostListItem(QWidget):
    ...
    def __init__(self, host: Host, connected: bool = False, parent: QWidget | None = None) -> None:
        ...
        proto_label.setFixedSize(40, 40)
        proto_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        proto_label.setStyleSheet(
            f"font-size: 10px; font-weight: 800; color: {proto_color}; "
            f"background-color: {Colors.BG_DARKER}; border-radius: 6px; "
            f"border: 1px solid {Colors.BORDER}; "
        )
        ...
        for tag in host.tags[:4]:
            pill = TagPill(tag.id or 0, tag.name, tag.color, style="outline")
            row2.addWidget(pill)
...
class HostListWidget(QWidget):
    ...
    def _on_new_group(self) -> None:
        from rlqshell.ui.vault.group_editor import GroupEditor
