...
class GoogleDriveProvider(AbstractCloudProvider):
    ...
    def connectivity_check_url(self) -> str:
        return f"{_API_URL}/about?fields=user"
    ...
    async def authenticate(self, auth_code: str) -> bool:
        ...
        async with session.post(_TOKEN_URL, data=data, proxy=self._proxy_url) as resp:
            if resp.status != 200:
    ...
    async def _ensure_folder(self) -> str:
        ...
        query = (
            f"name='{_APP_FOLDER}' and mimeType='application/vnd.google-apps.folder'"
            f" and trashed=false"
        )
        url = f"{_API_URL}/files?q={query}&fields=files(id,name)"
        async with session.get(
            url, headers=self._headers(), proxy=self._proxy_url
        ) as resp:
    ...
    async def upload_file(self, local_path: str, remote_path: str) -> None:
        ...
        if existing_id:
            ...
            async with session.patch(
                url, headers=headers, data=content, proxy=self._proxy_url
            ) as resp:
                if resp.status != 200:
            ...
            async with session.post(
                url, headers=headers, data=body, proxy=self._proxy_url
            ) as resp:
    ...
    async def list_files(self, folder: str) -> list[RemoteFileInfo]:
        ...
        async with session.get(
            ...
            return [
                RemoteFileInfo(
                    path=f"{folder}/{f['name']}", size=int(f.get("size", 0))
                )
